...
public class ObjectPropertyStatementTemplateModel extends PropertyStatementTemplateModel {
	...
	private String makeDeleteUrl() {
    	...
    	// WCMC Customized
        AbstractObjectPropertyStatementAction stmt =  new DropObjectPropertyStatement(
                vreq.getJenaOntModel(), subjectUri, property, 
                objectUri);
        if (log.isDebugEnabled()) {
	        log.debug("Drop Object Harvestedby data: " + subjectUri + " " + objectUri + " " + this.data.get("harvestedBy"));
	        Iterator<String> iter = this.data.keySet().iterator();
	        while (iter.hasNext()) {
	        	String key = iter.next();
	        	log.debug("Key: " + key + " Value: " + this.data.get(key));
	        }
        }
        
        stmt.setHarvestedBy(this.data.get("harvestedBy"));
        RequestedAction action =  stmt; 
	...
	private String makeEditUrl() {
       ...
       // Determine whether the statement can be edited
    	// WCMC customized
        AbstractObjectPropertyStatementAction stmt =  new EditObjectPropertyStatement(
                vreq.getJenaOntModel(), subjectUri, property, 
                objectUri);
        if (log.isDebugEnabled()) {
        	log.debug("Edit Object Harvestedby data: " + subjectUri + " " + objectUri + " " + this.data.get("harvestedBy"));
        }
        stmt.setHarvestedBy(this.data.get("harvestedBy"));
        RequestedAction action =  stmt;
